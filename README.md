...
| 状況                                                  | 止め方                                                                                                  |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| フォアグラウンドで `pnpm dev` 中                      | そのターミナルで `Ctrl + C`                                                                             |
| バックグラウンドで残った / ターミナルを閉じてしまった | `lsof -ti:8931 \| xargs kill` (まずは SIGTERM。最終手段で `kill -9`)                                    |
| 何が動いているか先に見たい                            | `lsof -i:8931` または `ps aux \| grep -E "tsx\|node.*serve" \| grep -v grep` で PID 確認 → `kill <PID>` |
| Docker で起動した                                     | `docker compose down`                                                                                   |
| tsx watcher だけまとめて落としたい                    | `pkill -f "tsx watch"`                                                                                  |

`kill -9` (SIGKILL) は SQLite の WAL が中途半端に残る可能性があるので最終手段。基本は無印 (SIGTERM) で落として、tsx / node のクリーンアップを走らせる。
